...
Before I go any further, I need to add a new scripting define to the project to enable Anna : `LATIOS_ADDON_ANNA` and bootstrap it in the `LatiosBootstrap` class.
```csharp
// Easy !
...
Since I'm having a hard time visualizing the World Bounds,
 I've written this small editor script to help me out:
...
Let's break it down:
- `Custom Collider`is a Psyshock authoring script. I've set the `Collider Type` to `Compound` and checked `Generate From Children` so that it will generate a compound collider from the children of the game object (obviously).
- `Collision Tag Authoring` is an Anna authoring script. Set to `Include environment Self Only`. It will add an `EnvironmentCollisionTag` to the baked entity, automatically generate an `EnvironmentCollisionLayer`, add it to the `sceneBlackBoardEntity` (so we can query it) and update it in `BuildEnvironmentCollisionLayerSystem`.  
...
Finally, I'll create the `PlayerInputSystem` that will read the input and update the `PlayerInputState` component.
```csharp
...
The `PlayerMovementSystem` will not run, it will nor update the RigidBody velocity but the physics simulation will still run and the player will be able to move.
...
thus the errors complaining about "disposed arrays" and "layers" being "different".
